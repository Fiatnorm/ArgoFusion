...
- `af -n` 必须先输出订阅面板链接，再按自适应、原始节点订阅、Base64、Clash/Mihomo、Sing-box 的顺序输出全部订阅链接、唯一一张自动适配订阅 QR 和明文节点；链接标签必须对齐，节点标题显示编号、标签和 `（协议+WS+TLS）` 类型。不得为其他订阅或单个节点重复输出 QR。自动适配订阅 QR 同时显示在网页订阅面板中，并作为单独的 `/auto-qr.svg` 订阅面板资源提供。
...
- `服务启停` 中选择“重启服务”后必须直接重启 Nginx、当前核心与 Argo Tunnel，不再进行二次确认。
...
- 主面板状态顺序固定为“Argo Tunnel、代理核心、WARP、节点概览、Argo 域名、优选入口、Argo 回源、组件版本”；节点概览按协议显示节点总数。只读页不显示 `0` 操作提示，普通返回静默，取消配置明确提示但不得写入半成品。所有交互子页面固定按“状态或说明 → 操作”分区，状态、配置范围、备份配置或风险提示在前，菜单或确认输入在后；底部用亮蓝分隔线收束。有现有值或默认值的表单统一使用 `Enter · 默认 | 0 · 取消`；核心切换页面和合并信息分区均命名为“核心配置”。参数配置中的节点表固定使用 18/8/24/7/21 显示宽度，节点列表标题前不得额外插入空行，协议显示为 VLESS、VMess、Trojan；直连节点显示 VPS 公网 IPv4，SOCKS5 节点只显示主机端口，绝不输出用户名或密码。节点增改删必须逐项校验，输入错误只重试当前步骤。订阅中心保持白底蓝字，指定格式卡片顺序固定为“原始节点订阅、Base64、Clash/Mihomo、Sing-box”。
- `af -x` 必须诊断 `config`、`data` 的 `700` 与 `subscriptions` 的 `755`；健康时汇总最近日志，出现 ERROR 或诊断失败时才展开相关日志。
- 展示 Xray 日志时必须过滤配置读取行以及 WebSocket、VMess、Trojan 的已知弃用提醒，保留实际运行过程与错误输出。
