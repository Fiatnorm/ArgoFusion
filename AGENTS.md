...
- 主面板状态顺序固定为“Argo Tunnel、代理核心、WARP、Argo 域名、优选入口、Argo 回源、组件版本”；只读页不显示 `0` 操作提示，普通返回静默，取消配置明确提示但不得写入半成品。节点表固定使用 14/7/18/5/12 显示宽度，协议显示为 VLESS、VMess、Trojan，并且 SOCKS5 只能展示 `direct`、`SOCKS5` 或主机端口，绝不输出用户名或密码。
