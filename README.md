...
v2.14.16 调整终端层级与配置保留语义：横幅恢复 `Sing-box / Xray`，系统环境摘要左移并与版本信息对齐；代理核心按“运行中 · Xray”显示，WARP 本地代理地址保持亮白，仅公网 IP 与优选入口 IP 使用亮紫。可交互页面将全局 `0` 提示收敛到标题右端，其中 `0` 使用亮黄、说明使用亮白；`af -n` 将节点 URI 区命名为“原始节点”，编号后的标签使用亮紫。订阅中心保持白底蓝字，并将“原始节点链接”置于指定格式首位。已存在的 Token、域名、优选入口和 UUID 在留空时保持当前值。
...
终端输出使用高亮配色：亮青字标和键名、亮紫页面标题、输入提示、当前核心、优选入口 IP 与公网 IP，亮蓝分区与分隔线、亮黄色菜单序号、停用状态及标题右端的导航 `0`，亮白承载主要内容（包括 WARP 本地代理地址和导航说明），绿/黄/红分别表示成功、警告和错误。订阅链接使用亮白下划线，`af -n` 的“原始节点”编号后标签使用亮紫。主面板、可返回菜单、可取消表单和只读页分别在标题右端显示对应的 `0 · …` 提示或不显示提示；普通返回不额外输出提示。已有配置项的输入展示当前值并标注“留空保持”，新安装的必填项仍拒绝留空；确认统一为“确认动作？`[y/N]`：”，诊断行明确显示“已通过 / 无效 / 未运行”。菜单、诊断、节点订阅与表格统一采用内容块布局：区块之间保留一行，区块内部保持紧凑。分隔线统一为 64 列 ASCII `-`，节点表按 14/7/18/5/12 显示宽度对齐且不显示 SOCKS5 用户名和密码；IPv6 优选入口在状态行中显示为 `[地址]:端口`；订阅中心保持白底蓝字，指定格式按“原始节点链接、Base64、Clash/Mihomo、Sing-box”排序；重定向输出、`TERM=dumb` 或设置 `NO_COLOR=1` 时自动关闭全部颜色和文本装饰。
