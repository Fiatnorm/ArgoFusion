...
v2.16.1 修复流量页面显示“当前核心计数暂不可读”且定时器停止的问题。采集器参照 ArgoX 从当前 Sing-box/Xray 运行配置读取 Stats API 地址，配置地址不可用时再探测当前核心进程的回环监听端口；同时兼容 Xray API 将 64 位计数返回为 JSON 数字或数字字符串。进入 `af -t` 时会恢复已安装但停止的 `afs-traffic.timer`，采集失败时显示具体原因，便于区分核心未运行、API 无响应和账本写入失败。
...
`af -t` 会先恢复已安装但停止的统计定时器，再从当前核心运行配置读取 Stats API 地址并采集计数；配置与实际监听错位时会探测当前核心进程的回环监听端口。随后显示自上次重置以来的持久统计。入站按 `nodes.conf` 的节点标签归属，可粗略表示每个节点的客户端上传、下载与合计；已删除但尚未重置的标签保留在“历史节点”区。出站分别显示共享 `direct`、共享 `warp` 和每个节点独立的 `socks-标签`。由于 direct 与 WARP 可能被多个节点共用，出站视图只能表示出口总量，不能反推各节点份额；每节点用量应以入站视图为准。采集失败时，统计状态会显示核心进程、Stats API 或 SQLite 对应的具体失败原因。
