...
终端输出使用高亮配色：亮青字标和键名、亮紫页面标题、输入提示、当前核心、优选入口 IP 与公网 IP，亮蓝分区与分隔线、亮黄色菜单序号、停用状态及标题右端的导航 `0`，亮白承载主要内容（包括 WARP 本地代理地址和导航说明），绿/黄/红分别表示成功、警告和错误。Token 已配置为绿色、未配置为黄色。订阅链接使用亮白下划线，`af -n` 的“原始节点”编号后使用亮紫标签并附加 `（协议+WS+TLS）`。主面板新增按协议计数的“节点概览”；可返回菜单、确认表单和只读页分别在标题右端显示对应提示或不显示提示。有现有值或默认值的表单统一显示 `Enter · 默认 | 0 · 取消`，其中 `Enter` 与 `0` 均为亮黄；输入项只展示方括号中的约束或值，不重复“按 Enter 保持”。确认统一为“确认动作？`[Y/n]`：”，支持大小写且留空表示确认；服务重启选择后直接执行，不二次确认。诊断行明确显示“已通过 / 无效 / 未运行”，日志隐藏 Xray 的配置读取以及 WebSocket、VMess、Trojan 弃用提醒。所有交互子页面均采用“状态或说明 → 操作”的内容块布局。分隔线统一为 64 列 ASCII `-`，节点表按 18/8/24/7/21 显示宽度对齐且标题前不额外插入空行；直连显示 VPS 公网 IPv4，SOCKS5 显示主机/IP:端口，绝不显示用户名和密码；IPv6 优选入口在状态行中显示为 `[地址]:端口`；订阅中心保持白底蓝字，指定格式按“原始节点订阅、Base64、Clash/Mihomo、Sing-box”排序；重定向输出、`TERM=dumb` 或设置 `NO_COLOR=1` 时自动关闭全部颜色和文本装饰。
